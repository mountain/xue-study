...
def masked_project(matrix, values, weights, masks):
    ...
    coeff=np.empty((nt,p)); gram=np.empty((nt,p,p)); residual=np.empty(nt)
    conditions=[]
    for times in groups.values():
        ...
        coeff[times]=c; gram[times]=g
        residual[times]=np.sum((y-c@b.T)**2*w,axis=1)
    return coeff,gram,residual,{'distinct_domains':len(groups),
...
def build(degree=6):
    ...
    for channel in CHANNELS:
        ...
        c,g,e,diag=masked_project(matrix,y,w,masks)
        size=c.shape[1]
        arrays[name+'_c']=c; arrays[name+'_gram']=g; arrays[name+'_residual']=e
