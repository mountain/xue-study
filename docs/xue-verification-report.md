...
### 2026-09-23 · 用户说「看来和上次一样」——把它变成了一次跨模式检验

#### 起因

用户看了 2026-09-22 运行的 θe850 渲染图，说「看来和上次一样」。
**这个观察本身就是一个对照**：真实的大气特征会随运行变化，结构性假象不会。

归档里有 **3 次 gfs 运行 + 2 次 ecmwf + 3 次 aifs** 的 θe850 poster，
所以这个对照可以做得比跨运行更强 —— **可以跨模式**。

#### 结果

```
编码上限 = offset 230 + 254×0.5 = 357 K（三个模式相同）

模式     运行          高原最大K/饱和%     暖池饱和%   中欧对照饱和%
gfs    2026091718    357.0 / 57.85%       0.03%       0.00%
gfs    2026091800    357.0 / 51.38%       0.08%       0.00%
gfs    2026091806    357.0 / 55.69%       0.25%       0.00%
ecmwf  2026091712    357.0 /  2.46%       0.00%       0.00%
ecmwf  2026091800    353.0 /  0.00%       0.01%       0.00%
aifs   2026091718    354.0 /  0.00%       0.00%       0.00%
aifs   2026091800    352.0 /  0.00%       0.00%       0.00%
aifs   2026091806    351.5 /  0.00%       0.00%       0.00%
```

#### 三条结论

1. **GFS 高原框有 51–58% 的格点顶在编码上限 357 K**，三次运行稳定（51.4 / 55.7 / 57.9）。
   **稳定 ⇒ 结构性，不是天气。**

2. **ECMWF 为 0.00–2.46%，AIFS 三次全为 0.00%。**
   **所以这个饱和是 GFS 特有的，不是三家通病。**

3. **对照区几乎不饱和**：暖池 0.00–0.25%（那是真正暖的地方），中欧 0.00%。
   **⇒ 天花板不是被真实的暖顶到的，是被假象顶到的。**

#### 为什么这比原先的结论更强

早先只知道「GFS 高原 366–371 K 而 ERA5 331.6 K」——两个数相差大。
现在知道的是：**在 xue 发布的三个模式里，GFS 是那个离群者，
而它把高原框一半以上的信息压成了同一个值。**

**编码上限一旦顶到，该区域的空间变化被完全抹掉。**
读这个产品的人看到的是一片平的高原。

#### 边界

- 只用了 **poster**（0.5° 降采样网格），不是全分辨率 store。全分辨率下
  饱和比例可能不同，未测。
- 三个模式的 poster 网格不一致，本步按各自 `xue:grid` 取框，未做重采样对齐。
- 「饱和」定义为 `value ≥ 上限 − 2 K`，阈值是选定的，不是从数据推出的。

### 2026-09-23 · 把高原热穹顶挪到地面之上重做：**假象消失，真信号留下**

#### 先解决仪器问题（这是前提，不是附注）

高原地面气压中位 **582.6 hPa**（本仓库实测），所以：

```
850 hPa   在地下（67.9% 的格点）  ← 那个「热穹顶」是地下外推值削平的产物
500 hPa   在地面之上（约 5500 m）  ← 可用
250 hPa   远在其上                 ← 归档中最高可用层次（无 200）
```

#### 结果：跨模式分歧在 500 hPa 上消失

```
500 hPa 温度 °C      高原 80-92E,26-32N   暖池 120-180E,15S-15N   中欧 0-20E,45-55N
aifs                    -4.2                  -4.2                 -14.3
ecmwf                   -3.5                  -4.0                 -14.0
gfs                     -3.8                  -4.7                 -14.3
──────────────────────────────────────────────────────────────────────────
...
**三个模式在高原上的分歧（2.0 K）与在暖池（1.0 K）、中欧（1.5 K）同量级。**

对照 850 hPa：GFS 与 ERA5 相差 **35–40 K**，且 GFS 高原框 **51–58%** 顶在编码上限。

> **所以高原不是模式天然分歧的地方。只要问一个【确实存在】的层次，三家就一致。**
> 分歧完全来自「向一个在地下的层次要值」这个要求本身。

#### 而且真信号在那里

**500 hPa 上高原（−3.8 ～ −4.2 °C）与暖池（−4.0 ～ −4.7 °C）几乎一样暖**，
而中欧同层次是 −14 °C —— 差 10 K。

**这是真实的「高原抬升热源」信号**（夏季高原作为抬升热源驱动亚洲季风），
**不需要任何假象就能看到**。它体现在温度对纬度的偏离上，而不是体现在绝对高温上。

#### 所以

```
「高原热穹顶」  在 500 hPa 上【真实】—— 是抬升热源，比同纬度暖约 10 K
                在 850 hPa 上【虚假】—— 地下外推值削平到编码上限
```

**这两件事必须分开说。** 混在一起会导致：拿 850 hPa 的图去研究真实现象的动力学，
而那张图的形状、范围、强度全部是人为的。

#### 第三次同一类错误（已改）

脚本里写 `q.get("unit","K")` —— 但 `q` 是**量化字典**，里面**没有 `unit` 键**，
于是默认成 K，对摄氏值又减了 273.15，得出 −273 °C 这种荒谬数。

**单位在 *variable* 字典里，不在 *quantization* 里。**

> 与早先 GRIB（GDAL 返回已是 °C，又减 273.15）、Zarr（xarray 已解码，又套量化）
> 是**同一类**：读取器/元数据已经给了正确的量纲，脚本又按自己的假设处理了一遍。
> **判据：数值荒谬（−273、1e8、−71）时，先查单位与解码，不要查物理。**

### 2026-09-23 · 可视化：把「目前能做的」做成七张图，并让每一张自己能被检验

#### 起因与范围

用户说「我们把目前能做的可视化做一下」。所以第一步不是画图，是**列出哪几条结论的数据在手上** ——
结果分成三类：本地 `archive/` 里能重算的、需要一次轻量外部读取的、以及只有记录没有数据的。
只有前两类进了图。**SAH 退出方案没进，因为它还是坏的**（`scripts/sah_exit.py`，见上一节），
坏的东西画不出对的图。

产出：`figures/` 下 7 张 PNG + 各自的 JSON，加一个看板 `figures/index.html`；
每张图由 `scripts/fig_*.py` 生成，页脚自带边界。**数字只从图自己写的 JSON 里抄进看板，不从散文重打。**

#### 先补的一步：poster 的经纬映射**从未被检验过**

前六张图里有四张要把 poster 画到地图上，而此前只有一条注释说「720×361, 0.5°, row-major,
north-to-south」—— **那是假设，不是检验**。画在错的角上不是「稍微不对」，
是**另一块地球的照片**。

`scripts/poster_grid_check.py` 拿归档的机场实测去检验它：把 poster 的 `tmp2m` 首帧
（＝该次运行的 0 时效分析）与运行时刻 ±60 分钟内的观测比，**扫遍南北极序、半格偏移与经度接缝**：

```
候选                      MAE     偏差
lat0=90, north_up, lon0=-180    1.70 K   -0.25 K   ← 成立
最强对手（经度滚动半周）          7.19 K             ← 被排除
```

**这是本仓库第一次把「poster 的方向」变成可失败的检验。** 4,982 条报告参与，最佳比最强对手好 5.49 K。

**一次自己踩到的坑，记下来**：这个检验的**第一版里，「经度滚动」这个候选写成了
`(lon + 180) % 360 - 180` —— 在 −180…180 的轴上这是恒等变换**，于是 roll=True 与 False
返回逐字节相同的 MAE。**一个与另一个候选完全相同的候选，分辨不了任何东西。**
这与 CONTINUE.md 里「守卫只挡一个方向等于没有守卫」是同一类：**候选集里可以有不会失败的成员。**
改成把接缝移到轴的中间之后，对手才真的成了对手（7.19 K）。

#### 图里出现了三条散文里没有的新事实

**一、GFS 高原框的 θe850，中位数就是编码上限本身。**

报告此前写「51–58% 的格点顶在 357 K 上限」，那是 `value ≥ 上限 − 2 K` 的口径。现场重算把口径收紧到
`value == 上限`：

```
运行           框内中位    饱和(≥上限−2K)   恰在上限码
2026091718      357.0 K      57.85%         53.23%
2026091800      355.0 K      51.38%         44.92%
2026091806      356.5 K      55.69%         49.85%
```

**中位数落在上限上**，意思是超过一半的格点共用一个码，而这些格点各自的真值都不同。

**二、xue 自己的色标根本到不了 θe850 所在的区间 —— 这一条是直接跑它的代码得到的。**

`web/src/palettes.ts` 的 `TEMPERATURE_STOPS` 是摄氏 −60…50，`buildPalette` 用**解码后的值**去查这张表。
θe850 的量化是 `230 + 0.5 × code`，即 230–357 K，**全部落在表的顶端之外**。
用该仓库自带的 esbuild 打包 `palettes.ts`、调用真的 `buildPalette`：

```
thetae850   255 个码位 →  1 种颜色   (112,20,65,255)  ← 整层一个常数色
tmp850      231 个码位 → 211 种颜色
tmp2m       221 个码位 → 221 种颜色
tmp500      201 个码位 → 151 种颜色
```

对照正常，所以不是这个检查坏了。

**它推翻了什么、没推翻什么，必须分开说。**
报告 §576 曾把一张浏览器截图当作「高原在图上更亮」的证据，并写明那是本地构建。
本步发现：**该代码路径下这一层是常数色，截图里的梯度不可能来自它**；
而 `thetae850` 在**本机 xue 检出的 `web/src` 里根本不存在**（全树 grep 无此标识符），
所以那张截图来自另一份已不存在的构建。

> **因此这条不是「§576 错了」，而是「§576 的适用范围到此为止」** ——
> 它记录的是一次真实浏览器渲染，但它不能用来支持「今天线上页面长什么样」。
> 与 ITS_LIVE 对岩体失明、机场站在 2000 m 以上只有 2 对同属一类：
> **限制不在数据量，在这次观测取自哪条路径。**

**三、「暖池对照」的数值取决于用哪台仪器，两个口径不能合并。**

现场重算给了一个**只换区域、其余全不换**的对照 —— 同样三次归档运行：
...
这一条比原先的跨系统对照更强，因为它把模式、时刻、解码链、量化全部固定住了。

**但同一个重算也暴露出一个此前没写下的分歧**：归档 poster 的暖池中位（341.5 / 342.8 / 343.0 K）
比记录来的 ERA5 与 MERRA-2 暖池值（338.64 / 338.63 K）**高约 3.8 K**。
原先那句「三者在暖池一致到 1.4 K」是**跨仪器**说的，与本步的**同仪器**对照不是同一个量。
图上把两者分开画、分开说，**没有并进同一个对照**。

#### 第四条：层次那条几何事实，现在由本仓库自己测出来了

此前「高原地面气压中位 582.6 hPa」是记录来的。本步直接读 ARCO ERA5 的 `surface_pressure`
与 `geopotential_at_surface`（`scripts/fig_level_problem.py`，2020-09，120 个时次）：

```
高原框 (80-92E, 26-32N)，32 格（ERA5 1.5° 网格）
  地面气压中位        574.3 hPa      （p10–p90 548.7–948.7）
  850 hPa 位于地下     78.1% 的格点   距地面中位 −3,247 m
  500 hPa 位于地下      0.0%          距地面中位 +1,106 m
  250 hPa 位于地下      0.0%          距地面中位 +6,298 m
对照 · 暖池 (120-180E, 15S-15N)：地面气压中位 1,009.9 hPa，850 hPa 在地下 0.12%
```

与记录的 582.6 hPa 差 8 hPa（时段与网格不同），与 MERRA-2 记录的 67.9% 同量级。
**结论方向与记录一致，但现在是本仓库的测量而不是引用。** 右侧面板另给了不依赖阈值的说法
（位势差 < 0），因为「地下」的判据是选定的。
...
| 死路 | 证据 |
|---|---|
| 取 Natural Earth 海岸线作底图 | 两个主机连续截断：raw.githubusercontent 给 38 KB 半截文件；带 `-C -` 续传后得到 140 KB、**且在第 40,169 字节处混入了一个 JPEG 文件头**。改用**写明坐标的地标**代替底图，图上也就不再依赖任何外部几何数据 |

#### 边界（本步所有图共享的）

- 所有地图类图基于 **poster**，是 2 倍降采样网格（0.5°），**不是全分辨率 store**；三个模式未做重采样对齐。
- 经纬映射已检验（1.70 K），但那只保证**平面到经纬度的方向**，不保证 poster 是模式的原始网格。
- 「饱和」的阈值 `上限 − 2 K` 是**选定值**，不是导出值；每张用到它的图都写明了。
- `figures/` 里 ERA5 与 MERRA-2 的数字标为 **RECORDED**（源为 ARCO ERA5 与 MERRA-2 M2T1NXSLV，
  报告 §2656），本步未复算；标为「本仓库实测」的才是现场解码重算的。
- 看板是**快照**，页面上写了生成时刻。

#### 未做

- **SAH 退出方案仍是坏的**，本步没有碰它 —— CONTINUE.md 上一节给的复原步骤（质心算完后
  直接打出前三个事件的 `clat/clon`，看是哪一天变 NaN）仍是下一步。
- 事件流的非事件侧、sounding/cma/mrms 进事件流、跨运行技巧曲线：都没动。
- 图只覆盖已有结论，**没有产生新的物理结论** —— 唯一的新事实是读数层面的（上限与色标）。

#### 第五、六张图把两个旧结论重新算了一遍，其中一条的样本量已经变了

`figures/discrimination-ceiling.png` 与 `figures/error-budget.png` 是把 §3042 / §3242 / §3449 三条
重新算出来的图。**重算暴露了四处需要写下来的分歧。**

**一、`archive/` 已经比 §3449 写它的时候大了约十九倍。**

```
§3449 记录（2026-09-18）      本次重算（2026-09-23）
观测 16,347 条                观测 304,922 条      （56 个 airport 批次，独立复核）
同格同刻测站对 n = 478        n = 4,261
代表性 sd 0.96 K              0.94 K
方差份额 20% / 5% / 75%       19.3% / 5.5% / 75.2%
```

**采集器一直在跑，所以报告里那个「478 对」是一个会过期的数。**
结论方向没变（模式仍占约 75%），但**此后引用 §3449 必须带上它当时的数据量**，
否则引的是一个已经不再成立的样本。

**二、海拔 2000 m 以上那一条，两个数都对，单位不同。**

§3507 写「只有 2 对测站」，本次在 0.25° 下数出 **17 条配对观测** ——
**17 条观测 = 2 组不同的测站对**（K7BM–KAEJ 在科罗拉多，15 个小时各一条；
SETU–SKIP 在厄瓜多尔，2 条）。所以 §3507 的「2 对」是**不同测站对**的口径，得到确认。
图上两个数都印出来，柱子仍画成空槽（样本不足），**脚本自己算出的 0.51 K 只打印不上图** ——
2 组独立测站对远低于该脚本自设的 15 对下限。

**三、判决的那个摆动，可以复现成「同一条分布被读在 0.47σ 与 2.00σ 两处」。**

把 §3242 记录的 7 时效（中位 −0.30 K、sd 2.13 K、n = 4,196）当作**一条正态**：

```
记录值     36.2%   （阈值 1.00 K = 0.47σ）
正态预测   35.8%   ← 差 0.4 个百分点
记录值     94.5%   （阈值 4.26 K = 2.00σ）
正态预测   95.2%   ← 差 0.7 个百分点
```

**两个判决数都在同一条正态的预测之内。** 所以「36.2% 还是 94.5%」这个摆动
**不携带关于 GFS 的信息** —— 它是同一个分布被两条阈值线切出来的两个数。
这比 §3242 原来说的「判决取决于标准」更强：不但是**取决于标准**，而且**两个数完全由标准决定**。

顺带补上一个报告里没有的数：**用本步实测的代表性误差（0.94 K）作第二档标准，同一次比对读作 67.8%** ——
它既不是仪器标称，也不是数据自身的散布，是中间那一档。

**四、`36.2% / 94.5%` 这次复算不了，原因在上游。**

三个归档 gfs 运行的 0.25° store（`gfs.2026091718|1800|1806/tmp2m.zarr/zarr.json`）
**现在上游都返回 HTTP 404**。0.25° 档的原始散布因此取不到，
图上把这一档明确标为 **RECORDED（报告 §3242 表第 3257 行）**，并在数据落地的 0.5° 档上给出可比读数
（32.7% / 73.1% / 95.5%，run 2026091806，n = 4,977）。**两者不可混为一谈：网格不同。**

**五、一个脚本里的数已经作废但脚本没改。**

`scripts/discrimination_ceiling.py` 的 `POPULATIONS` 里**仍然写着已被撤回的 `564*11 = 6204`**；
更正只存在于 `claims.toml` 与 §3108。本步**没有改那个脚本**（不在授权范围内），
但在图的 stdout 与图面上都标了出来。**一个脚本与它自己的 claim 说法不一致，是下一次会踩的坑。**

图上同时给出两个口径的倍数，因为「报警率是基率的数千倍」这句话只在撤回的口径下成立：

```
                    PDD 91%         雨-冰 95%
撤回口径 (N=6,204)   5,646×          5,894×      ← §3081 的数，已复现
更正口径 (N=33–44)      30–40×         31–42×
```
